...
# Installs a CI-produced self-hosted Ruffle bundle as an isolated v5 test
# runtime. It deliberately does not restart PM2/Caddy, touch /, or replace
# ruffle-dbr-v3/ruffle-dbr-v4.

project_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
content_dir="${project_dir}/src/client/content/localhost"
destination="${content_dir}/ruffle-dbr-v5"
source_path="${1:-}"

if [[ -z "${source_path}" || ! -e "${source_path}" ]]; then
  printf 'Usage: %s <ruffle-dbr-v5 artifact directory | .tar.gz archive>\n' "$0" >&2
  exit 64
fi

parent_dir="$(dirname "${destination}")"
staging="$(mktemp -d "${parent_dir}/.ruffle-dbr-v5.staging.XXXXXX")"
...
grep -F 'Runtime: v5' "${staging}/DUNGEON_BLITZ_BUILD.txt" >/dev/null || {
  echo 'Artifact is not marked as the v5 runtime' >&2
  exit 65
}
...
printf 'Installed Ruffle v5 test runtime: %s\n' "${destination}"
[[ -z "${backup}" ]] || printf 'Previous v5 runtime retained at: %s\n' "${backup}"
printf 'Test only at: https://dungenblitz.ecliptia.net/play-test/?v=5&renderer=wgpu-webgl\n'
